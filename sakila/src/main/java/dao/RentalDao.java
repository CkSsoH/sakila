...
public class RentalDao {
	public List<Map<String, Object>> selectRentalSearchList(int storeId, String customerName, String beginDate, String endDate, int beginRow, int rowPerPage){
		...
		try {
			...
			if(storeId == -1 && beginDate.equals("") && endDate.equals("")) { //스토어 ID와 대여일, 반납일 모두 선택X
				sql += " ORDER BY rental_id limit ?,?"; // 해당 쿼리 추가
				stmt = conn.prepareStatement(sql);
				stmt.setString(1, "%"+customerName+"%");
				stmt.setInt(2, beginRow);
				stmt.setInt(3, rowPerPage);
			} else if (storeId == -1 && beginDate.equals("") && !endDate.equals("")) {// 대여일만 선택 ( 스토어 ID, 반납일 선택 X,(현재일까지로검색))
			...
			} else if (storeId == -1 && !beginDate.equals("") && endDate.equals("")) {// 반납일만 선택 ( 스토어ID, 대여일 선택 X,(대여일 임의설정해서 넣음))
				...
				stmt.setInt(3, beginRow);
				stmt.setInt(4, rowPerPage);
			} else if (storeId == -1 && !beginDate.equals("") && !endDate.equals("")) {// 스토어 ID 선택 X ( 일자 선택 O)
			...
			} else if (storeId != -1 && beginDate.equals("") && endDate.equals("")) { // 스토어 ID 선택 (날짜공백)
				...
				stmt.setInt(3, beginRow);
				stmt.setInt(4, rowPerPage);
			} else if (storeId != -1 && beginDate.equals("") && !endDate.equals("")) { //스토어 ID 선택, 반납일자 선택(대여일 미선택)
				...
				stmt.setInt(4, beginRow);
				stmt.setInt(5, rowPerPage);
			} else if (storeId != -1 && !beginDate.equals("") && endDate.equals("")) { //스토어 ID 선택, 대여일선택(반납일 미선택)
				...
				stmt.setInt(4, beginRow);
				stmt.setInt(5,rowPerPage);
			} else if (storeId != -1 && !beginDate.equals("") && !endDate.equals("")) { //스토어 ID, 대여일,반납일 모두 선택
